// Shared comparable-sales quarry valuation engine.
// Used by the weekly build-valuation-estimates backend function AND the
// marketplace UI so both sides derive the same comp-based range.
//
// This is marketplace screening intelligence from comparable sales / regional
// market benchmarks — NOT an appraisal, reserve estimate, or guaranteed
// recoverable mineral value.

import { rockCategoryFor } from "./rockTypes.js";
...
// Regional default per-acre market benchmarks for Southeast US quarry/mineral
// land, used only when no comparable sales match. These are order-of-magnitude
// market references — far above tax-assessed farmland values, which is what
// made the old anchor method undervalue quarry properties.
...
// Rank comparable sales against a target site. State match is required;
// rock category, status, permit, acreage similarity, recency, and verification
// quality add to the score. Returns the top matches (best first).
...
// Comp-based valuation. Returns the same shape as the legacy
// calculateIndicativeQuarryValue so cards render without changes.
