      ...
      <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6">
        ...
        {selectedCompany && <section className="mt-5 rounded-3xl border border-sky-200 bg-card p-5 shadow-lg sm:p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            <Link to={`/network/community?tab=opportunities`} className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2.5 text-xs font-bold"><Target className="h-4 w-4"/>Post opportunity</Link>
        ...
        {tab === "companies" && <section className="mt-6">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{filteredCompanies.slice(0, 120).map((company) => <article key={company.key} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            ...
            <button type="button" onClick={() => setSelectedCompanyKey(company.key)} className="mt-4 inline-flex w-full items-center justify-between rounded-xl border border-border px-4 py-2.5 text-xs font-bold">Open company network <ArrowRight className="h-4 w-4 text-sky-700"/></button>
          </article>)}</div>
