...
export default function CompanyNetworkDetail() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setOpportunities((opportunityRows || []).filter((row) => row.status !== "Closed" && (companyKey(row.author_company) === targetKey || linkedIds.has(row.linked_mining_site_id))));
    ...
    <header className="border-b border-slate-800 bg-slate-950 text-white" style={{ paddingTop: "env(safe-area-inset-top, 16px)" }}>
      <div className="mx-auto max-w-7xl px-4 pb-7 pt-4 sm:px-6">
        ...
        <div className="mt-6 flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">{user?.id ? <button onClick={toggleWatch} disabled={watchBusy} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-bold text-slate-950 disabled:opacity-50">{watch ? <BellRing className="h-4 w-4"/> : <Bell className="h-4 w-4"/>}{watch ? "Watching" : "Watch company"}</button> : <Link to={`/login?returnTo=${encodeURIComponent(window.location.pathname + window.location.search)}`} className="rounded-xl bg-white px-4 py-2.5 text-xs font-bold text-slate-950">Sign in to watch</Link>}<Link to={`/network/deals/new?company=${encodeURIComponent(company.name)}`} className="rounded-xl border border-slate-700 px-4 py-2.5 text-xs font-bold">Post opportunity</Link></div>
        </div>
      </div>
    </header>
