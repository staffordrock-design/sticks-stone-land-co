        ...
        {tab === "feed" && <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
          ...
          <section className="min-w-0 space-y-4"><form onSubmit={createPost} className="rounded-2xl border border-border bg-card p-4 shadow-sm"><div className="flex gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-900 text-sm font-bold text-white">{(publicProfile?.full_name || privateProfile?.full_name || user?.email || "S").slice(0,1).toUpperCase()}</div><div className="min-w-0 flex-1"><textarea value={body} onChange={(e) => setBody(e.target.value)} maxLength={1500} placeholder="Share an industry update, equipment need, project, hiring need or question…" className="min-h-24 w-full resize-none rounded-xl border border-border bg-background p-3 text-sm outline-none focus:ring-2 focus:ring-sky-300" /><div className="mt-3 flex items-center justify-between gap-3"><select value={postType} onChange={(e) => setPostType(e.target.value)} className="rounded-lg border border-border bg-background px-3 py-2 text-xs font-bold">{POST_TYPES.map((type) => <option key={type}>{type}</option>)}</select><button disabled={posting || !body.trim()} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-sm font-bold text-white disabled:opacity-40">{posting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Post</button></div></div></div></form>
            {visiblePosts.map((post) => {
              const postComments = commentsFor(post);
              return <article key={post.id} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex min-w-0 gap-3">
                    ...
                    <div className="min-w-0"><div className="truncate font-bold">{post.author_name || "Industry member"}</div><div className="truncate text-xs text-muted-foreground">{[post.author_headline, post.author_company].filter(Boolean).join(" · ")}</div><div className="mt-0.5 text-[11px] text-muted-foreground">{post.created_at ? new Date(post.created_at).toLocaleString() : ""}</div></div>
                  </div>
                ...
                <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-border pt-3">
                  <button type="button" onClick={() => toggleLike(post)} disabled={reactingPostId === post.id} className={`inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-semibold disabled:opacity-50 ${myReactionFor(post) ? "text-rose-700" : "text-muted-foreground"}`}><Heart className={`h-4 w-4 ${myReactionFor(post) ? "fill-current" : ""}`} />{myReactionFor(post) ? "Liked" : "Like"}{reactionCountFor(post) > 0 ? ` · ${reactionCountFor(post)}` : ""}</button>
                  <span className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-semibold text-muted-foreground"><MessageCircle className="h-4 w-4" />{postComments.length} {postComments.length === 1 ? "comment" : "comments"}</span>
                  {post.author_user_id !== user?.id && <Link to={`/messages?user=${encodeURIComponent(post.author_user_id)}`} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-semibold text-muted-foreground"><Send className="h-4 w-4" />Message</Link>}
                ...
                {postComments.length > 0 && <div className="mt-3 space-y-2 border-t border-border pt-3">{postComments.slice(-5).map((comment) => <div key={comment.id} className="flex gap-2 rounded-xl bg-muted/50 p-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-700 text-xs font-bold text-white">{(comment.author_name || "M").slice(0,1).toUpperCase()}</div><div className="min-w-0"><div className="text-xs font-bold">{comment.author_name || "Industry member"}</div><div className="mt-0.5 whitespace-pre-wrap text-sm leading-5">{comment.body}</div></div></div>)}</div>}
                <form onSubmit={(event) => { event.preventDefault(); createComment(post); }} className="mt-3 flex items-center gap-2">
                  <input value={commentDrafts[post.id] || ""} onChange={(event) => setCommentDrafts((current) => ({ ...current, [post.id]: event.target.value }))} maxLength={700} placeholder="Write a comment…" className="min-h-10 flex-1 rounded-xl border border-border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-sky-300" />
                  <button type="submit" disabled={commentingPostId === post.id || !String(commentDrafts[post.id] || "").trim()} className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-slate-900 text-white disabled:opacity-40" aria-label="Post comment">{commentingPostId === post.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</button>
                </form>
              </article>;
            })}
