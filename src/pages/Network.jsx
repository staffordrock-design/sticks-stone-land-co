          ...
          <section className="mt-7">
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              {visibleOpportunities.map((item) => {
                ...
                return <article key={item.id} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-3"><div><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${item.opportunity_type === "Looking For" ? "bg-sky-100 text-sky-900" : "bg-emerald-100 text-emerald-900"}`}>{item.opportunity_type}</span><h3 className="mt-3 font-heading text-xl font-bold">{item.title}</h3></div>{item.confidentiality === "NDA / Confidential" && <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-700"><LockKeyhole className="h-3 w-3" />NDA</span>}</div>
