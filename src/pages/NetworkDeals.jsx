...
export default function NetworkDeals() {
  ...
  return <div className="min-h-screen bg-slate-50 pb-28 dark:bg-background">
    ...
    <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">{filtered.map((item) => { const site = siteById.get(item.linked_mining_site_id); const mine = myInterestFor(item); const incoming = incomingCountFor(item); return <article key={item.id} className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><Pill>{item.opportunity_type}</Pill><h3 className="mt-3 font-heading text-xl font-bold">{item.title}</h3><div className="mt-1 text-xs text-muted-foreground">{[item.author_name,item.author_company].filter(Boolean).join(" · ")}</div></div>{item.confidentiality === "NDA / Confidential" && <div className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-bold text-amber-900"><ShieldCheck className="h-3 w-3"/>NDA</div>}</div><p className="mt-4 text-sm leading-6 text-muted-foreground">{item.description}</p><div className="mt-4 flex flex-wrap gap-2">{item.states && <Pill>{item.states}</Pill>}{item.commodities && <Pill>{item.commodities}</Pill>}{item.asset_types && <Pill>{item.asset_types}</Pill>}{mine && <Pill>My interest: {mine.status}</Pill>}{incoming > 0 && <Pill>{incoming} buyer interest{incoming === 1 ? "" : "s"}</Pill>}</div>{site && <div className="mt-4 rounded-xl border border-border bg-muted/20 p-4"><div className="flex items-center gap-2"><Mountain className="h-4 w-4 text-sky-700"/><strong className="text-sm">{site.mine_name}</strong></div><div className="mt-1 text-xs text-muted-foreground">{[site.county,site.state,site.operator_name].filter(Boolean).join(" · ")}</div></div>}<div className="mt-5 flex flex-wrap gap-2 border-t border-border pt-4"><Link to={`/network/deals/${item.id}`} className="rounded-xl bg-slate-900 px-4 py-2.5 text-xs font-bold text-white">Open deal workspace</Link>{site && <Link to={`/mines/${site.id}`} className="rounded-xl border border-border px-3 py-2.5 text-xs font-bold">Linked quarry intelligence</Link>}</div></article>; })}</div>
      {filtered.length === 0 && <div className="mt-4 rounded-2xl border border-dashed border-border p-10 text-center"><Target className="mx-auto h-8 w-8 text-muted-foreground"/><div className="mt-3 font-bold">No matching deals yet</div><p className="mt-1 text-sm text-muted-foreground">Post a buyer requirement or quarry offering to start the transaction network.</p><Link to="/network/deals/new" className="mt-4 inline-flex rounded-xl bg-slate-900 px-4 py-2.5 text-xs font-bold text-white">Post a deal</Link></div>}
    </main>
  </div>;
}
