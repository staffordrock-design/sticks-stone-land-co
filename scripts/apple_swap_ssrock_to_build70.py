...
REPORT_PATH = Path("reports/apple_swap_ssrock_build70.json")
...
def submit_review(c: ASC, review_id: str) -> None:
    ...
    report["actions"].append("Submitted build 70 + app version + $199 subscription + subscription group version")
    save()
...
def verify(c: ASC, version_id: str, review_id: str, build_id: str, sub_version_id: str, group_version_id: str) -> None:
    ...
    if attached.get("id") != build_id or final["attached_build_number"] != TARGET_BUILD:
        raise RuntimeError("Final verification failed: build 70 is not attached")
    if final["attached_build_processing_state"] != "VALID":
        raise RuntimeError("Final verification failed: attached build 70 is not VALID")
...
def main() -> None:
    ...
    try:
        ...
        if (current.get("attributes") or {}).get("version") == TARGET_BUILD:
            if reviews:
                report["actions"].append("Build 70 was already attached; no review swap was needed")
                report["final"]["review_state"] = (reviews[0].get("attributes") or {}).get("state")
            save()
            print("SUCCESS: S&S Rock Holdings is already attached to build 70.")
            return
        ...
        print("SUCCESS: S&S Rock Holdings review now targets build 70 with the custom icon and $199 subscription.")
